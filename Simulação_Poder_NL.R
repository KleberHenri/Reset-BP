# Simulação do tamanho (n=50, mu^2) ---------------------------------------
set.seed(144)
...
write.table(result100, file = "Mu2EtatestTam50_PA.txt", sep = " ")

qestimado100=rbind(quantilRV,quantilE,quantilW,quantilG) # quantilRV,quantilE,quantilW
write.table(qestimado100, file = "qestimMu2Etatest50_PA.txt", sep = " ")


# Simulação do Poder (n=50, mu^2)------------------------------------------------------
set.seed (144)
...
X=cbind(x0,x01) #matriz de regressores de mu
eta1=X%*%beta
mu = exp(eta1^1.8) # função de ligação raiz
...
write.table(resultPoder50, file = "Mu2EtatestPoderNL50_PA.txt", sep = " ")
